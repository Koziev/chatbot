...
Генерирует датасет с тройками ПРЕДПОСЫЛКА-ВОПРОС-ОТВЕТ, например:
...
Также генерируется датасет для обучения модели интерпретации текста, которая должна
из двух фрагментов (вопроса и неразвернутого ответа) восстановить полный ответ:

```
глаза кто открыл?
Оля|Оля открыла глаза
```

Результат интерпретации отделяется от краткого ответа символом вертикальной верты.
