# -*- coding: utf-8 -*-
'''
Посимвольная генерация всех слов ответа. В отличие от qa_chargenerator.py, слова генерируются
...
trim_answer = 4 # ограничение на число слов в ответе
...
def generate_rows(sequences, targets, batch_size, mode, max_nb_answer_words):
    batch_index = 0
    batch_count = 0
    ...
    y_batch = [np.zeros((batch_size, max_outword_len, output_dims), dtype=np.bool) for _ in range(max_nb_answer_words)]

    while True:
        for irow, (seq, answer_words) in enumerate(itertools.izip(sequences, targets)):
            vectorize_words(seq[0], X1_batch, batch_index, word2vec)
            vectorize_words(seq[1], X2_batch, batch_index, word2vec)
            for iword, answer_word in enumerate(answer_words):
                for ichar, c in enumerate(answer_word):
                    y_batch[iword][batch_index, ichar, char2id[c]] = True
            ...
            if batch_index == batch_size:
                ...
                if mode == 1:
                    outputs = dict(('output{}'.format(i), y_batch[i]) for i in range(max_nb_answer_words))
                    yield ({'input_words1': X1_batch, 'input_words2': X2_batch}, outputs)
                else:
                ...
                X1_batch.fill(0)
                X2_batch.fill(0)
                for i in range(max_nb_answer_words):
                    y_batch[i].fill(0)
                    #y_batch.fill(0)
                batch_index = 0
...
class VisualizeCallback(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, batch, logs={}):
        ...
        for step, batch in enumerate(generate_rows(self.val_input, self.val_output, BATCH_SIZE, 1, max_nb_answer_words)):
            if step == nb_steps:
                break
            ...
            for iy in range(len(y_pred)):
                ...
                for iword in range(max_nb_answer_words):
                    v = y_batch['output{}'.format(iword)][iy]
                    target_chars = self.decode_str(v)

                    v = y_pred[iword][iy]
                    predicted_chars = self.decode_str(v)

                    true_words.append(target_chars)
                    predicted_words.append(predicted_chars)

                nb_samples += 1
                correct_sample = True
                predicted_str = u' '.join(predicted_words).strip()
                true_str = u' '.join(true_words).strip()
                ...
                if nb_shown < 10:
                    print(
                        colors.ok + '☑ ' + colors.close if correct_sample else colors.fail + '☒ ' + colors.close,
                        end='')

                    print(u'true={:50s} model={}'.format(true_str, predicted_str))
                    nb_shown += 1
...
max_nb_answer_words = 0  # макс. число слов в ответе
...
for i,record in df.iterrows():
    ...
    if answer not in [u'да', u'нет']:
        ...
        words = tokenizer.tokenize(answer)[:trim_answer]  # для отладки отсекаем первые два слова ответа!
        max_outword_len = max(max_outword_len, max(map(len,words)))
        max_nb_answer_words = max(max_nb_answer_words, len(words))
...
print('max_nb_answer_words={}'.format(max_nb_answer_words))
...
model_config = {
                ...
                'max_nb_answer_words': max_nb_answer_words,
                'w2v_path': w2v_path,
...
if RUN_MODE == 'train':
    ...
    print('Building the NN computational graph: {} {}'.format(NET_ARCH, encoder_arch))
    ...
    # Теперь добавляем вторую часть сетки - посимвольные декодеры слов ответа.
    ...
    for iword in range(max_nb_answer_words):
        ...
        decoder = TimeDistributed(Dense(nb_chars, activation='softmax'), name='output{}'.format(iword))(decoder)
        decoders.append(decoder)

    model = Model(inputs=[words_net1, words_net2], outputs=decoders)
    ...
    for index, row in tqdm.tqdm(df.iterrows(), total=df.shape[0], desc='Extract phrases'):
        ...
        if answer not in [u'да', u'нет']:
            ...
            answer_words = rpad_wordseq(tokenizer.tokenize(answer)[:trim_answer], max_nb_answer_words)

            input_data.append((premise_words, question_words, premise, question))
            output_data.append([rpad_charseq(word, max_outword_len) for word in answer_words])

            #if len(input_data) >= 5000: # DEBUG!
            #    break
    ...
    hist = model.fit_generator(generator=generate_rows(train_input, train_output, batch_size, 1, max_nb_answer_words),
                               steps_per_epoch=nb_train_patterns//batch_size,
                               epochs=200,
                               verbose=2,
                               callbacks=callbacks,
                               validation_data=generate_rows(val_input, val_output, batch_size, 1, max_nb_answer_words),
