...
if run_mode == 'train':
    ...
    samples1 = list(filter(lambda s: len(s.phrases) == 2, samples1))
    ...
    samples = list(filter(lambda s: len(s.phrases) == 2, samples))
    ...
    with codecs.open(os.path.join(tmp_folder, 'nn_interpreter.validation.txt'), 'w', 'utf-8') as wrt:
...
# Дальше идут классы для алгоритма декодера
...
if run_mode == 'query':
    ...
    lemma2forms = dict()
    with codecs.open(os.path.join(data_folder, 'word2lemma.dat'), 'r', 'utf-8') as rdr:
        for line in rdr:
            tx = line.strip().split('\t')
            if len(tx) == 3:
                word = tx[0].lower()
